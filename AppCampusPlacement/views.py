...
def AddCompany(request):
    if request.method == "POST":
        ...
        Package = request.POST.get('pack', '')
        File = request.FILES.get('File', None)
        Pre_Placement = request.POST.get('placement1', None)  # Use 'placement1' as per the form field name
        Test_Date = request.POST.get('test_date1', None)
        ...
        # Example:
        if not Pre_Placement:
            messages.error(request, "Pre Placement date is required.")
            return render(request, 'AddCompany.html', {})

        # Save the data to the database
        data = CompanyDetails11(
